...
export class EditPaymentDetailsPage implements OnInit {
  ...
  constructor(
    ...
    private paymentMethodUser: PaymentMethodsService,
    ...
    this.route.queryParams.subscribe(params => {
      if (this.router.getCurrentNavigation().extras.state) {
        ...
        this.payment_method = new paymentMethodClass(
          this.paymentMethod.method_id,
          this.paymentMethod.method_name
        );
        this.meth_id=this.payment_method.method_id;
        console.log(this.payment_method);
        console.log(";amfkllsd"+this.meth_id);
      }
    });
    this.edit_payment_form = new FormGroup({
      card_holder_name: new FormControl(
        this.paymentMethod.user_name,
        Validators.required
      ),
      card_name: new FormControl(
        this.paymentMethod.card_name,
        Validators.required
      ),
      card_no: new FormControl(this.paymentMethod.card_no, [
        Validators.required,
        Validators.minLength(12),
        Validators.maxLength(12),
        Validators.pattern(/^-?(0|[1-9]\d*)?$/)
      ]),
      expiry_m: new FormControl(this.paymentMethod.expiry_month, [
        Validators.required,
        Validators.max(12),
        Validators.min(1)
      ]),
      expiry_y: new FormControl(this.paymentMethod.expiry_year, [
        Validators.required,
        Validators.min(19),
        Validators.max(25)
      ]),
  ...
  submit() {
    var myUser = new paymentMethodsUser(
      this.paymentMethod.user_id,
      this.paymentMethod.payment_id,
      this.paymentMethod.user_name,
      this.edit_payment_form.get("card_no").value,
      this.edit_payment_form.get("expiry_m").value,
      this.edit_payment_form.get("expiry_y").value,
      "",
      this.edit_payment_form.get("method_type").value,
      this.edit_payment_form.get("card_name").value
    );
    this.paymentMethodUser.updatePaymentUser(myUser).subscribe(
      async (data: any) => {
        console.log(data);
        if (data.affectedRows == 1) {
          ...
          this.router
            .navigateByUrl("/view-payment-method", navigationExtras)
            .then(() => {
              this.ngOnInit();
            });
          // this.ngOnInit();
  ...
  ngOnInit() {
    ...
    this.paymentMethodUser.getAllPaymentMethods().subscribe(
      (data: paymentMethodClass[]) => {
